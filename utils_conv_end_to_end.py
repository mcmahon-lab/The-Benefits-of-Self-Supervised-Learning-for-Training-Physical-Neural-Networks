...
def save_weights(BASE_PATH, prefix, net, epoch):
    '''
    Store the first 32 weights of the first weight matrix
    '''
    fig, axs = plt.subplots(4, 8, figsize=(4, 4))  # Create a 10x10 subplot grid

    for i, ax in enumerate(axs.flat):
        ax.imshow(net.layers[0].weight[i,0,:].view(5,5).detach().cpu(), cmap= "gray")  # Plot the weight matrix
...
def store_checkpoint(BASE_PATH, args, net, epoch, loss):
    '''
...
def pretrain(net, train_loader, train_layer = 1):
    '''
    ...
    net.eval()
    loss_tot = 0
    for batch_idx, (data, _) in enumerate(tqdm(train_loader, position = 0, leave = True)):
        net.optimizer.zero_grad()
        data = net.single_batch(data) #set the first dim to be n_views*batch_size
        data = data.to(net.device) #put on the GPU
        
        loss = net(data) #the loss is computed at each layer in the forward function
        loss.backward()
        loss_tot += loss.item()
        net.optimizer.step()
...
def pretraining_loop(BASE_PATH, args, net, train_loader, train_loader_classifier, test_loader, epochs = 20):
    ...
    for epoch in tqdm(range(epochs), position = 0, leave = True):
        net, loss = pretrain(net, train_loader)
        loss_tot.append(loss)

        store_checkpoint(BASE_PATH, args, net, epoch,  loss)

        if epoch%50 == 0: #train linear classifiers every 50 epochs to see the convergence
            print("training linear classifier")
            training_classifiers(BASE_PATH, args, net, epoch, train_loader_classifier, test_loader)
...
class Network_class(nn.Module):
    ...
    def __init__(self, args, net):
        ...
        size_input = (net.size_feature_map[-1]**2)*net.channels_list[-1]
        self.fc1 = nn.Linear(size_input, 10, bias = True)
...
def train_classifier(net, net_class, train_loader_classifier):
    '''
    ...
    for batch_idx, (data, target) in enumerate(train_loader_classifier):
        ...
        with torch.no_grad():
            output  = net.forward_simple(data)
...
def test_classifier(net, net_class, test_loader):
    '''
...
def training_classifier_loop(args, net, train_loader_classifier, test_loader):
    '''
    Function used to instantiate a linear probe on each layer, train it and store the corresponding training curves
    '''
    net_class = Network_class(args, net)
    train_loss, train_err = [], []
    test_loss, test_err = [], []

    for k in range(args.epochs_classifier):
        error, loss = train_classifier(net, net_class, train_loader_classifier)
        train_loss.append(loss)
        train_err.append(error.item())
        
        error, loss = test_classifier(net, net_class, test_loader)
...
def training_classifiers(BASE_PATH, args, net, epoch, train_loader_classifier, test_loader):
    '''
    Train a linear classifier on top of each layer - store the corresponding training curves
    '''
    net.eval()
    
    train_err, test_err, train_loss, test_loss=training_classifier_loop(args, net, train_loader_classifier, test_loader)
    name_dataframe = "linear_classifier"+ "_epoch#" + str(epoch) + ".csv"
