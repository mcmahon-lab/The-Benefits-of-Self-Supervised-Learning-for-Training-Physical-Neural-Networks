...
def pretrain(net, train_loader):
    ...
    for batch_idx, (data, _) in enumerate(tqdm(train_loader, position = 0, leave = True)):
        ...
        data1, data2 = data[0].to(net.device), data[1].to(net.device) #set the first dim to be n_views*batch_size

        _, loss = net(data1, data2) #the loss is computed at each layer in the forward function
        loss.backward()
        loss_tot += loss.item()
        net.optimizer.step()
...
def pretraining_loop(BASE_PATH, args, net, train_loader, train_loader_classifier, test_loader, epochs = 20):
    ...
    for epoch in tqdm(range(epochs), position = 0, leave = True):
        ...
        store_checkpoint(BASE_PATH, args, net, epoch, 0,  loss)
...
class Network_class(nn.Module):
    ...
    '''
    def __init__(self, args, net, layer):
        super(Network_class, self).__init__()
        
        size_input = (net.size_feature_map[layer+1]**2)*net.channels_list[layer+1]
...
def train_classifier(net, net_class, train_loader_classifier, input_layer = 0):
    ...
    for batch_idx, (data, target) in enumerate(train_loader_classifier):
        ...
        with torch.no_grad():
            states  = net.forward_simple(data)
        
        output = net.flatten(states[input_layer])
        y = net_class(output) 
...
def training_classifier_loop(args, net, train_loader_classifier, test_loader, input_layer):
    '''
    Function used to instantiate a linear probe on each layer, train it and store the corresponding training curves
    '''
    net_class = Network_class(args, net, input_layer)
...
def training_classifiers(BASE_PATH, args, net, epoch, train_loader_classifier, test_loader, layer):
    ...
    train_err, test_err, train_loss, test_loss=training_classifier_loop(args, net, train_loader_classifier, test_loader, layer)
