    ...
    with torch.no_grad():
        ...
        for batch_idx, (datas, _) in enumerate(tqdm(train_loader, position = 0, leave = True)): #now datas has a len of n_average!
            ...
            for idx, data in enumerate(datas):
                ...
                #print(perturbation)
                pos_obj = net(data, train_layer, perturbation) #the loss is computed at each layer in the forward function
                neg_obj = net(data, train_layer, -1*perturbation) #the loss is computed at each layer in the forward function
                
                #print(pos_obj, neg_obj, pos_obj-neg_obj)
