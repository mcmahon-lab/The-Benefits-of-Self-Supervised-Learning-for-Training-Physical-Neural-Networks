    ...
    with torch.no_grad():
        ...
        for batch_idx, (datas, _) in enumerate(tqdm(train_loader, position = 0, leave = True)): #now datas has a len of n_average!
            ...
            for idx, data in enumerate(datas):
                ...
                neg_obj = net.losses[train_layer].sim_coeff*neg_obj_repr + net.losses[train_layer].std_coeff* neg_obj_std + net.losses[train_layer].cov_coeff*neg_obj_cov
                
                #print(pos_obj_repr, pos_obj_std, pos_obj_cov)
                #print(neg_obj_repr, neg_obj_std, neg_obj_cov)
                #print(pos_obj_repr-neg_obj_repr, pos_obj_std-neg_obj_std, pos_obj_cov-neg_obj_cov)
                
                if idx == 0: #for the first "mini-mini-batch" we compute the gradient
                    grads = net.compute_spsa(perturbation, pos_obj, neg_obj, layer = train_layer)
                    grads_repr = net.compute_spsa(perturbation, pos_obj_repr, neg_obj_repr, layer = train_layer)
                    grads_std = net.compute_spsa(perturbation, pos_obj_std, neg_obj_std, layer = train_layer)
                    grads_cov = net.compute_spsa(perturbation, pos_obj_cov, neg_obj_cov, layer = train_layer)
            ...
            grads_avg = net.losses[train_layer].sim_coeff * grads_repr + net.losses[train_layer].std_coeff*grads_std +   net.losses[train_layer].cov_coeff*grads_cov
            grads /= args.n_average #average the resulting gradient
            grads_repr /= args.n_average
            grads_std /= args.n_average
            grads_cov /= args.n_average
            grads_avg /= args.n_average
            
            #print(grads)
            #print(grads_avg)
                  
            net.optimizer.zero_grad()
            net.apply_spsa(grads_avg, layer = train_layer) #apply the computed grad to the corresponding parameters of the network
            
            loss_tot += (pos_obj-neg_obj).item()
